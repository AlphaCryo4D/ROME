                             ...
                             int _padding_factor/* = 2*/,int _interpolator/* = TRILINEAR*/,int _r_min_nn/* = 10*/)
{
    ...
    data.fini();
    data.init(dimz, pad_size, pad_Fsize, true);
}
...
void MyProjector::computeFourierTransformMap(Vol<FDOUBLE>& ref_in,int _ori_size,int _current_size,FDOUBLE* power_spectrum,bool do_gridding/* = true*/)
{
    ...
    Faux.init(_ref_dim==3?padoridim:1, padoridim, padoridim/2+1);
    //
                                 ...
                                 int _blob_order/* = 0*/, double _blob_radius/* = 1.9*/, double _blob_alpha/* = 15*/)
{
    ...
    int dimz = ref_dim==3?pad_size:1;
    weight.fini();
    weight.init(dimz, pad_size, pad_Fsize, true);
...
#undef COMPLEX
        ...
        Vol<FDOUBLE> sum_weight;sum_weight.init(my_weight.dimz,my_weight.dimy,my_weight.dimx);
        Vol<MKL_Complex> sum_data;sum_data.init(my_data.dimz,my_data.dimy,my_data.dimx);
...
void MyBackProjector::convoluteBlobRealSpace(Vol<MKL_Complex>& Fconv, FourierTransformerBase& transformer, bool do_mask /*= false*/)
{
    ...
    Mconv.init(Fconv.dimz,Fconv.dimy, Fconv.dimy);
...
void MyBackProjector::windowToOridimRealSpace(Vol<MKL_Complex> &Fconv, Vol<FDOUBLE> &Vol_out, FourierTransformerBase& transformer)
{
    ...
    if (ref_dim == 2) {
        Ftmp.init(1, padoridim, padoridim/2+1);
    }
    else{
        Ftmp.init(padoridim, padoridim, padoridim/2+1);
    }
    ...
    Fconv.fini();
    if (ref_dim == 2) {
        Mout.init(1, padoridim, padoridim);
    }
    else{
        Mout.init(padoridim, padoridim, padoridim);
    }
    ...
    if (ref_dim == 2)
    {
        MoutCenter.init(1, padoridim, padoridim);
    }
    else
    {
        MoutCenter.init(padoridim, padoridim, padoridim);
    }
    ...
    {
        centerFFT3D(Mout.wptr(),MoutCenter.wptr(),padoridim,true);
    }
    TIMEPOINT
#ifdef DEBUG_WINDOWORIDIMREALSPACE
    // tt()=Mout;
                                    ...
                                    std::string tmp_folder/* = "NULL"*/)
{
    ...
    TIMEPOINT
    if(ref_dim == 3){
        Fconv.init(pad_size,pad_size,pad_Fsize);
        Fweight.init(pad_size,pad_size,pad_Fsize);
        Fnewweight.init(pad_size, pad_size, pad_Fsize);
    }
    else{
        Fconv.init(1,pad_size,pad_Fsize);
        Fweight.init(1,pad_size,pad_Fsize);
        Fnewweight.init(1, pad_size, pad_Fsize);
    }
    TIMEPOINT
