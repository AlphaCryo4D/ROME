...
#ifndef MAP_OPTIMIZERBASE_OLD_H_
#define MAP_OPTIMIZERBASE_OLD_H_
...
namespace MapOptimizerBase_old
{
	...
	//
    // ------------      base variables         	 -------------- //
#define MAPOPTIMIZER_OLD_BASE_VARS \
    /* (TYPE		, NAME							,VALUE ) */	\
...
#define AUTOREFINE_BASE_VARS \
    ...
    ELT(double		, best_resol_thus_far			, 0		)	SEP	\
...
#define ELT(T,N,V) extern T N;
    MAPOPTIMIZER_OLD_BASE_VARS
#undef SEP
#undef ELT
    
    // ------------ variable for expectation step -------------- //
#define MAPOPTIMIZER_OLD_EXP_VARS \
    ...
    //
    // ------------ 					thread variable 		-------------- //
#define MAPOPTIMIZER_OLD_THREAD_VARS \
...
#define MAPOPTIMIZER_OLD_HIDDEN_VAR_ELT \
        ...
        ELT(double	, smallest_changes_optimal_classes			, 9999999	)			SEP \
...
#undef SEP
                                      ...
                                      const MetaDataTable& metadata,bool do_split_random_halves)
    {
        ...
        {
            if (node==nodes-1) {// master node
                nr_global_images = metadata.numberOfParticles();// all particles...
                nr_local_images = 0;
                first_local_image = 0;
                last_local_image = 0;
            }
            else if(node < (nodes-1)/2)// half one
            {
                nr_global_images = metadata.numberOfParticles(1);
                nr_local_images = divide_equally(nr_global_images, (nodes-1)/2, node, first_local_image, last_local_image);
            }
            else// half two
            {
                int nr_global_images_half_one = metadata.numberOfParticles(1);
                nr_global_images = metadata.numberOfParticles(2);
                nr_local_images = divide_equally(nr_global_images, (nodes-1)/2, node-(nodes-1)/2, first_local_image, last_local_image);
            }
        }
        else
        {
...
#define SAME_IMAGE_DIVISION
    ...
    void gatherMetaDataToMaster(MetaDataTable& metadata,int masterNode = 0,bool do_split_random_halves = false);
    
#endif
    
}
#endif /* defined(MAP_OPTIMIZER_BASE_OLD_H_) */
