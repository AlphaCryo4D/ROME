...
void FourierShellTranslation
::setCoarseImageBoundary(const int* _Mresol_coarse, int _size)
{
...
void FourierShellTranslation
::setNormCorrectionBoundary(const int* _Mresol_fine, int _size)
{
	changeUid();
	...
	int index = 0;
	for (int res = min_iRes; res <= max_iRes; res++) {
		for (int i = 0; i < oldFsize2Fine; i++) {
			if (_Mresol_fine[i] == res) {
				nIndexFine[index] = i;
				iResFine[index] = res;
				index++;
			}
...
void FourierShellTranslation
::setScaleCorrectionBoundary(const VectorOfArray2d<FDOUBLE>& data_vs_prior_class)
{
	changeUid();
	...
	for (int iclass = 0; iclass < nr_class; iclass++)
	{
        ...
        MASTERNODE
        {
            if (thresholdResCount == 0) {
                std::cout << "WARNING : " << __FILE__ << " " << __LINE__ << " data_va_prior has no larger than 3!!!" << std::endl;
                // for (int ires = min_iRes; ires <= max_iRes; ires++)
                //    MASTERNODE std::cout<<data_vs_prior_class[iclass][ires]<<" ";
                // MASTERNODE std::cout<<std::endl;
            }
            bool no_continuous = false;
            ...
            if (no_continuous) {
                std::cout << "WARNING : " << __FILE__ << " " << __LINE__ << " data_va_prior is not monotone decreasing!!!" << std::endl;
                std::cout << "WARNING : " << " TODO TODO .... excluding <=3 points.... " <<std::endl;
                // TODO : add excluding index if data_vs_prior is not monotone decreasing
...
//
__declspec(noinline) void ParticleModel::getLargeShiftedMaskImageOneTile(int iimage,FDOUBLE* Fimgs_shifted_mask_real,FDOUBLE* Fimgs_shifted_mask_imag,
...
__declspec(noinline) void ParticleModel::getLargeShiftedNomaskImageOneTile(int iimage,FDOUBLE* Fimgs_shifted_nomask_real,FDOUBLE* Fimgs_shifted_nomask_imag,
                                                      int n_start,int n_end,int itrans)
{
    assert(iimage<exp_nr_images);
    // Shift through phase-shifts in the Fourier transform
...
void __declspec(noinline) ParticleModel::shiftOneFimageByXYabTable(const FDOUBLE* Fimgs_in_real,const FDOUBLE* Fimgs_in_imag,
