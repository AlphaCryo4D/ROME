...
#pragma omp parallel for
        for (int thread = 0; thread < nr_threads; thread++) {
            thread_data[thread].init(pad_size, pad_size, pad_Fsize, false/*true*/);
            thread_weight[thread].init(pad_size, pad_size, pad_Fsize, false/*true*/);
        }
    }
}
...
void MAPModel::updateCurrentResolution(std::vector<VectorOfFDOUBLE>& data_vs_prior_class,bool set_by_ini_high,int& nr_iter_wo_resol_gain)
{
