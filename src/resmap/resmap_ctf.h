...
public:
                         ...
                         bool do_abs = false, bool do_only_flip_phases = false, bool do_intact_until_first_peak = false, bool do_damping = true) const
    {
        ...
        double retval;
        if (do_intact_until_first_peak && fabs(argument) < PI/2.)
        {
            retval = 1.;
        }
        else
    ...
    {
        if (fabs(X) < ROME_EQUAL_ACCURACY &&
            fabs(Y) < ROME_EQUAL_ACCURACY)
            return 0;
    ...
    void getFftwImage(double *result, int dim,int orixdim, int oriydim, double angpix,
                      bool do_abs = false, bool do_only_flip_phases = false, bool do_intact_until_first_peak = false, bool do_damping = true);
    
    void getFftwImage(float *result, int dim,int orixdim, int oriydim, double angpix,
                      bool do_abs = false, bool do_only_flip_phases = false, bool do_intact_until_first_peak = false, bool do_damping = true);
