...
void HealpixSampler::readFromSampling(std::string fn_sampling,bool reset_sampling,bool debug_flag /*= false*/)
{
    ifstreamCheckingExistence samplingFile(fn_sampling.c_str());
    {
        ...
        while (true) {
            if (startingRead) {
                ...
                int new_hp_order;
                double new_psi_step,new_trans_step,new_trans_range;
...
#define CINMETADATASTR(V) samplingFile >> line;samplingFile >> stringTmp;if(debug_flag) {MASTERNODE std::cout<<std::setw(30)<<line<<" "<<stringTmp<<std::endl;}
                ...
                CINMETADATADOUBLE(	"HealpixOrder"				)if(reset_sampling) new_hp_order = doubleTmp;
                CINMETADATASTR(		"SymmetryGroup"				)
                CINMETADATADOUBLE(	"TiltAngleLimit"			)
                CINMETADATADOUBLE(	"PsiStep"					)if(reset_sampling) new_psi_step = doubleTmp;
                CINMETADATADOUBLE(	"OffsetRange"				)if(reset_sampling) new_trans_range = doubleTmp;
                CINMETADATADOUBLE(	"OffsetStep"				)if(reset_sampling) new_trans_step = doubleTmp;
                ...
                //
                if(reset_sampling){setTranslations(new_trans_step,new_trans_range);setOrientations(new_hp_order, new_psi_step);}
                //
...
void SamplingGrid::sortDirection()
{
    sorted_idir.clear();
#if 0
    typedef std::vector<double> Direction;
    std::vector<Direction> half_one_dir;
    std::list<Direction> half_two_dir;
    for (int idir = 0; idir < exp_over_rot.size(); idir++) {
        if (exp_over_rot[idir][0] >= 0) half_one_dir.push_back({(double)idir,exp_over_rot[idir][0], exp_over_tilt[idir][0]});
        else half_two_dir.push_back({(double)idir,exp_over_rot[idir][0], exp_over_tilt[idir][0]});
    }

    for (int i = 0; i < half_one_dir.size(); i++) {
        auto& my_dir = half_one_dir[i];
        sorted_idir.push_back(my_dir[0]);
        // find the opposite direction
        // every positive direction will have a corresponding negative direction
        // by sorting direction will increase the reuse of slice of 3D volume
        for (auto opposite_dir : half_two_dir) {
            if (fabs(my_dir[1]-180-opposite_dir[1])<10e-10 && fabs(my_dir[2]+opposite_dir[2]-180)<10e-10)
            {
                sorted_idir.push_back(opposite_dir[0]);
                half_two_dir.remove(opposite_dir);
                break;
            }
        }
    }
#else
    for (int idir = 0; idir < exp_over_rot.size(); idir++) {
        sorted_idir.push_back(idir);
    }
#endif
    ERROR_CHECK(sorted_idir.size()!=exp_over_rot.size(), "");
//    for (const auto& idir : sorted_idir) {
//        std::cout<<idir<<" "<<exp_over_rot[idir][0]<<" "<<exp_over_tilt[idir][0]<<std::endl;
//    }
}
...
void SamplingGrid::computeGrid3D(HealpixSampler& sampler3d,int _current_oversampling)
{
    ...
    if (current_oversampling==0) sortDirection();
...
//#define DEBUG_LOCAL_SEARCH
        ...
        std::cout<<prior_rot<<" "<<prior_tilt<<" "<<prior_psi<<std::endl;
        std::cout<<sigma_rot<<" "<<sigma_tilt<<" "<<sigma_psi<<std::endl;
...
int SamplingGrid::exp_nr_local_psi()
{
    int nr_images = pointer_psi_nonzeroprior.size();
    int max_nr_psi = 0;
    bool diff_nr_psi_for_some_image=false;
    for (int iimage = 0; iimage < nr_images; iimage++) {
        int iimage_nr_psi = pointer_psi_nonzeroprior[iimage].size();
        if(iimage_nr_psi > max_nr_psi){
            // the images may have different nr_psi!!!
            if (max_nr_psi) diff_nr_psi_for_some_image = true;
            max_nr_psi = iimage_nr_psi;
        }
    }
    // MASTERNODE if(diff_nr_psi_for_some_image) std::cout<<"some images have different nr_psi."<<std::endl;
    return max_nr_psi;
}

int SamplingGrid::exp_nr_local_dir()
{
    int nr_images = pointer_dir_nonzeroprior.size();
    int max_nr_dir = 0;
    bool diff_nr_dir_for_some_image=false;
    for (int iimage = 0; iimage < nr_images; iimage++) {
        int iimage_nr_dir = pointer_dir_nonzeroprior[iimage].size();
        if(iimage_nr_dir > max_nr_dir){
            // the images may have different nr_dir!!!
            if (max_nr_dir) diff_nr_dir_for_some_image = true;
            max_nr_dir = iimage_nr_dir;
        }
    }
    // MASTERNODE if(diff_nr_dir_for_some_image) std::cout<<"some images have different nr_dir."<<std::endl;
    return max_nr_dir;
}

std::tuple<int, int, double> SamplingGrid::getDirPsiPdf(int iimage,int local_dir,int local_psi)
{
    int nr_dir = pointer_dir_nonzeroprior[iimage].size();
    int nr_psi = pointer_psi_nonzeroprior[iimage].size();
    if (local_dir >= nr_dir || local_psi >= nr_psi) {
        return std::tuple<int,int,double>(-111111,-111111,-111111);
    }
    int global_idir = pointer_dir_nonzeroprior[iimage][local_dir];
    int global_ipsi = pointer_psi_nonzeroprior[iimage][local_psi];
    double pdf = directions_prior[iimage][local_dir]*psi_prior[iimage][local_psi];
    return std::tuple<int,int,double>(global_idir,global_ipsi,pdf);
}

void SamplingGrid::setOrientation(HealpixSampler& sampler3d,int _current_oversampling,
                                  ...
                                  FDOUBLE* &over_rot,FDOUBLE* &over_tilt,FDOUBLE* &over_psi)
{
    ...
    // for local searching,the pass-in idir and ipsi  is local index
    // so need to convert this index to global index
    if (do_local_search_iimage > -1) {
        auto dir_psi_pdf = getDirPsiPdf(do_local_search_iimage, idir, ipsi);
        int new_idir = std::get<0>(dir_psi_pdf);
        int new_ipsi = std::get<1>(dir_psi_pdf);
        idir = new_idir;ipsi = new_ipsi;
    }
...
#define USE_VECTOR
    ...
    FDOUBLE prior_direction_vec[3], my_direction_vec[3], sym_direction_vec[3], best_direction_vec[3];
#endif
#if defined(USE_MAT) && defined(USE_VECTOR)
    auto checkDiff = [&](FDOUBLE* vec1,FDOUBLE* vec2,int length,std::string info) {
        for (int i = 0; i < length; i++) {
            if (fabs(vec1[i]-vec2[i])>10e-10) {
                std::cout.precision(15);
                std::cout<<"Diff "<<info<<" in selectOrientationsWithNonZeroPriorProbability : "<<vec1[i]<<" "<<vec2[i]<<" "<<fabs(vec1[i]-vec2[i])<<std::endl;
                ERROR_REPORT("");
            }
        }
    };
    ...
    if (is_3D)
    {
        ...
        for (int idir = 0; idir < nrPix; idir++)
        {
            ...
            {
#if defined(USE_MAT)
                Matrix1D<FDOUBLE> prior_direction_mat(3), my_direction_mat(3), sym_direction_mat(3), best_direction_mat(3);
                // Get the direction of the prior
                Euler_angles2direction(prior_rot, prior_tilt, prior_direction_mat.data().data());
                // Get the current direction in the loop
                Euler_angles2direction(rot_angles[idir], tilt_angles[idir], my_direction_mat.data().data());
                // Loop over all symmetry operators to find the operator that brings this direction nearest to the prior
                FDOUBLE best_dotProduct = dotProduct(prior_direction_mat, my_direction_mat);
                best_direction_mat = my_direction_mat;
                for (int j = 0; j < R_repository.size(); j++)
                {
                    sym_direction_mat =  L_repository[j] * (my_direction_mat.transpose() * R_repository[j]).transpose();
                    FDOUBLE my_dotProduct = dotProduct(prior_direction_mat, sym_direction_mat);
                    if (my_dotProduct > best_dotProduct)
                    {
                        best_direction_mat = sym_direction_mat;
...
#if defined(USE_VECTOR)
                // Get the direction of the prior
                Euler_angles2direction(prior_rot, prior_tilt, prior_direction_vec);
                // Get the current direction in the loop
                Euler_angles2direction(rot_angles[idir], tilt_angles[idir], my_direction_vec);
                // Loop over all symmetry operators to find the operator that brings this direction nearest to the prior
                FDOUBLE best_dotProduct_vec = (prior_direction_vec[0]*my_direction_vec[0])+(prior_direction_vec[1]*my_direction_vec[1])+(prior_direction_vec[2]*my_direction_vec[2]);
                best_direction_vec[0] = my_direction_vec[0];best_direction_vec[1] = my_direction_vec[1];best_direction_vec[2] = my_direction_vec[2];
                assert(R_repository[0].dimx()==3);assert(R_repository[0].dimy()==3);
                for (int j = 0; j < R_repository.size(); j++)
                {
                    auto R_repository_j_wptr = R_repository[j].wptr();
                    FDOUBLE temp0 = my_direction_vec[0]*R_repository_j_wptr[0*3+0]+my_direction_vec[1]*R_repository_j_wptr[1*3+0]+my_direction_vec[2]*R_repository_j_wptr[2*3+0];
                    FDOUBLE temp1 = my_direction_vec[1]*R_repository_j_wptr[0*3+1]+my_direction_vec[1]*R_repository_j_wptr[1*3+1]+my_direction_vec[2]*R_repository_j_wptr[2*3+1];
                    FDOUBLE temp2 = my_direction_vec[2]*R_repository_j_wptr[0*3+2]+my_direction_vec[1]*R_repository_j_wptr[1*3+2]+my_direction_vec[2]*R_repository_j_wptr[2*3+2];
                    auto L_repository_j_wptr = L_repository[j].wptr();
                    sym_direction_vec[0] = L_repository_j_wptr[0*3+0]*temp0+L_repository_j_wptr[0*3+1]*temp1+L_repository_j_wptr[0*3+2]*temp2;
                    sym_direction_vec[1] = L_repository_j_wptr[1*3+0]*temp0+L_repository_j_wptr[1*3+1]*temp1+L_repository_j_wptr[1*3+2]*temp2;
                    sym_direction_vec[2] = L_repository_j_wptr[2*3+0]*temp0+L_repository_j_wptr[2*3+1]*temp1+L_repository_j_wptr[2*3+2]*temp2;
                    FDOUBLE my_dotProduct = (prior_direction_vec[0]*sym_direction_vec[0])+(prior_direction_vec[1]*sym_direction_vec[1])+(prior_direction_vec[2]*sym_direction_vec[2]);
                    if (my_dotProduct > best_dotProduct_vec)
                    {
                        best_direction_vec[0] = sym_direction_vec[0];best_direction_vec[1] = sym_direction_vec[1];best_direction_vec[2] = sym_direction_vec[2];
                        best_dotProduct_vec = my_dotProduct;
                    }
                }
#endif
#if defined(USE_MAT) && defined(USE_VECTOR)
                checkDiff(&best_dotProduct,&best_dotProduct_vec,1,"best_dotProduct");
                checkDiff(best_direction_mat.data().data(),&best_direction_vec[0],3,"best_dotProduct");
#endif
                if (sigma_rot > 0. && sigma_tilt > 0.)
                {
#if defined(USE_MAT)
                    FDOUBLE diffang_mat = acosd( dotProduct(best_direction_mat, prior_direction_mat) );
#endif
#if defined(USE_VECTOR)
                    FDOUBLE diffang = acosd(best_direction_vec[0]*prior_direction_vec[0]+best_direction_vec[1]*prior_direction_vec[1]+best_direction_vec[2]*prior_direction_vec[2]);
#endif
#if defined(USE_MAT) && defined(USE_VECTOR)
                    checkDiff(&diffang_mat,&diffang,1,"diffang");
#endif
...
#if defined(USE_MAT)
                    Euler_direction2angles(best_direction_mat.data().data(), best_rot, best_tilt);
#endif
#if defined(USE_VECTOR)
                    Euler_direction2angles(best_direction_vec, best_rot, best_tilt);
#endif
