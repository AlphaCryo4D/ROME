...
void MLModel::initialize(int _ori_size,int _nr_classes,int _nr_groups,int _nr_directions,double sigma2_angle)
{
    ...
    if (sigma2_angle != 0) {
        ...
        sigma2_rot = sigma2_tilt = sigma2_psi = sigma2_angle;
    }
    //
...
#define CINMETADATA(V) modelFile >> line;modelFile >> doubleTmp;if(debug_flag) {MASTERNODE std::cout<<std::setw(30)<<line<<" "<<doubleTmp<<std::endl;}
                ...
                if(fabs(doubleTmp-mapModel.pixel_size)>1e-2){
                    std::cerr<<std::setprecision(12)<<doubleTmp<<" "<<std::setprecision(12)<<mapModel.pixel_size<<std::endl;
...
#undef CINMETADATA
    ...
    for (int igroup = 0; igroup < nr_groups; igroup++)
    {
        ...
        if(!metadata_fit){
            MASTERNODE std::cout<<"ml_model half-size is not correct for sigma2_noise[igroup],make sure you use -continue in autorefine!!!!";
            break;
        };
    }
    // data_model_pdf_orient_class_*
