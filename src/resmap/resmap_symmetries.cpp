...
int SymList::read_sym_file(FileName fn_sym)
{
    ...
    char line[80];
    char *auxstr;
    FDOUBLE ang_incr, rot_ang;
    int  fold;
    Matrix2D<FDOUBLE> L(4, 4), R(4, 4);
    Matrix1D<FDOUBLE> axis(3);
    ...
    if ((fpoii = fopen(fn_sym.c_str(), "r")) == NULL)
    {
        ...
        if (isSymmetryGroup(fn_sym, pgGroup, pgOrder))
        {
            ...
            ERROR_REPORT((std::string)"SymList::read_sym_file:Can't open file: "
    ...
    char* token_signal;// make token working on parallel
    for (int n=0; n<fileContent.size(); n++)
    {
    	strcpy(line,fileContent[n].c_str());
        auxstr = firstToken(line,token_signal);
        if (auxstr == NULL)
        {
            std::cout << line;
...
// Get matrix ==============================================================
void SymList::get_matrices(int i, Matrix2D<FDOUBLE> &L, Matrix2D<FDOUBLE> &R)
const
{
    int k, l;
    L.initZeros(4, 4);
...
// Set matrix ==============================================================
void SymList::set_matrices(int i, const Matrix2D<FDOUBLE> &L,
                           const Matrix2D<FDOUBLE> &R)
{
    int k, l;
    for (k = 4 * i; k < 4*i + 4; k++)
        for (l = 0; l < 4; l++)
...
// Add matrix ==============================================================
void SymList::add_matrices(const Matrix2D<FDOUBLE> &L, const Matrix2D<FDOUBLE> &R,
                           int chain_length)
{
    if (L.dimx() != 4 || L.dimy() != 4 || R.dimx() != 4 || R.dimy() != 4)
        ERROR_REPORT( "SymList::add_matrix: Transformation matrix is not 4x4");
    if (TrueSymsNo() == SymsNo())
    {
        __L.resize((__L.dimy()) + 4, 4);
        __R.resize((__R.dimy()) + 4, 4);
...
{
    Matrix2D<FDOUBLE> I(4, 4);
    I.initIdentity();
    Matrix2D<FDOUBLE> L1(4, 4), R1(4, 4), L2(4, 4), R2(4, 4), newL(4, 4), newR(4, 4);
...
#undef DEBUG
#ifdef DEBUG
           std::cout << "Matrix size " << tried.dimx() << " "
            << "trying " << i << " " << j << " "
...
#undef DEBUG
            ...
            tried.resize((tried.dimy()) + 1, (tried.dimx()) + 1);
        }
    }
}
...
bool SymList::isSymmetryGroup(FileName fn_sym, int &pgGroup, int &pgOrder)
{
   ...
   fn_sym_tmp=pathGetFilename(fn_sym);
   ...
   std::vector<std::string> &fileContent)
{
	...
	if (pgGroup == pg_CN)
    {
    	fileContent.push_back("rot_axis " + std::to_string((long long)pgOrder) + " 0 0 1");
    }
    ...
    else if (pgGroup == pg_SN)
    {
        ...
        fileContent.push_back("rot_axis " + std::to_string((long long)order) + " 0 0 1");
        fileContent.push_back("inversion ");
    }
...
FDOUBLE SymList::non_redundant_ewald_sphere(int pgGroup, int pgOrder)
{
    if (pgGroup == pg_CN)
    {
        return 4.*PI/pgOrder;
