        ...
        for (int i = 0; i < vol_out.dimy; i++) {
            for (int j = 0; j < vol_out.dimx; j++) {
                ...
                if (k2 > -1 && k2 < vol_in.dimz &&
                    i2 > -1 && i2 < vol_in.dimy &&
                    j2 > -1 && j2 < vol_in.dimx)
                    vol_out(k,i,j) = vol_in(k2,i2,j2);
                else
                    vol_out(k,i,j) = init_value;
            }
        }
    }
}
        ...
        for (int k = 0; k < vol.dimz; k++)
            for (int i = 0; i < vol.dimy; i++)
                for (int j = 0; j < vol.dimx; j++)
                {
                    ...
                    {
                        raisedcos = 0.5 + 0.5 * cos(rome_pi * (radius_p - r) / cosine_width );
                        sum += raisedcos;
                        sum_bg += raisedcos * vol(k, i, j);
                    }
                }
        sum_bg /= sum;
    }
        ...
        for (int i = 0; i < Min.dimy; i++)
            for (int j = 0; j < Fxsize; j++)
            {
                ...
                if (spectrum_type == AMPLITUDE_SPECTRUM)
                    spectrum[idx] += sqrt(Faux_real(k,i,j)*Faux_real(k,i,j)+Faux_imag(k,i,j)*Faux_imag(k,i,j));
                else
                    spectrum[idx] += Faux_real(k,i,j)*Faux_real(k,i,j)+Faux_imag(k,i,j)*Faux_imag(k,i,j);
                count[idx] += 1.;
            }
