...
------- example
when run the code,set
--datastream_in /Users/yongbeima/Documents/xcode_project/rome12_171025/DerivedData/rome12_171025/Build/Products/Debug/relion_data_stream/_relion_autorefine.dat
--datastream_out /Users/yongbeima/Documents/xcode_project/rome12_171025/DerivedData/rome12_171025/Build/Products/Debug/rome_data_stream/_rome_autorefine.dat
If you run auto-refine,the compare result can be found on :
/Users/yongbeima/Documents/xcode_project/rome12_171025/DerivedData/rome12_171025/Build/Products/Debug/relion_data_stream/_relion_autorefine.dat1_compare_result.txt
/Users/yongbeima/Documents/xcode_project/rome12_171025/DerivedData/rome12_171025/Build/Products/Debug/relion_data_stream/_relion_autorefine.dat2_compare_result.txt
the rome datastream will be writed on :
/Users/yongbeima/Documents/xcode_project/rome12_171025/DerivedData/rome12_171025/Build/Products/Debug/rome_data_stream/_rome_autorefine.dat1
/Users/yongbeima/Documents/xcode_project/rome12_171025/DerivedData/rome12_171025/Build/Products/Debug/rome_data_stream/_rome_autorefine.dat1
!!!!!! by the way,you need change the code on relion_src/ml_optimiser.h to write out it's datastream
--------------*/
...
class DataStream{
    template<typename T>
    int isSame(T* v1,T* v2,int len,double precision,std::ostream& os = std::cout){
        os.precision(20);
        int differentCounts = 0;
        for (int i = 0; i < len; i++){
            if(std::fabs(double(v1[i]-v2[i])) > precision) {
                if (differentCounts<10) {
                    os<<"(i:"<<i<<",v1:"<<v1[i]<<",v2:"<<v2[i]<<")"<<" ";
                }
                differentCounts++;
            }
        }
...
#include "../src/resmap/resmap_util_heap_defs.h"
    ...
    std::ofstream compare_file;
    std::vector<Ptr*> cachedPtr;
    size_t stream_size;
public:
    bool doDataStream = false;
    bool doDataStream_prior = false;
public:
    ...
    void init(std::string out_fn,std::string in_fn){
        ...
        // open file
        if(instream_fn!="NULL") {
            instream_file = fopen(instream_fn.c_str(),"rb");
            doDataStream = true;doDataStream_prior = true;
            std::string compare_file_name = instream_fn + "_compare_result.txt";
            compare_file.open(compare_file_name.c_str());
        }
        if(outstream_fn!="NULL") {outstream_file = fopen(outstream_fn.c_str(),"wb");doDataStream = true;doDataStream_prior=true;}
        std::cout.precision(20);

    }
    void fini(){
        if(instream_fn!="NULL") {
            fclose(instream_file);
            compare_file.close();
        }
    ...
    void turnOnPrior(){if(instream_fn!="NULL" || outstream_fn!="NULL") doDataStream_prior = true;}
    void turnOffPrior(){if(instream_fn!="NULL" || outstream_fn!="NULL") doDataStream_prior = false;}
    inline void foutDouble(double* _data,int _len,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(DoublePtr::make(_data,_len,_what,_file,_line));
    }
    inline void foutDoubleComplex(const std::complex<double>* _data,int _len,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(DoubleComplexPtr::make(_data,_len,_what,_file,_line));
    }
    inline void foutDouble(double _data,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(DoubleVar::make(_data,1,_what,_file,_line));
    }
    inline void foutFloat(float* _data,int _len,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(FloatPtr::make(_data,_len,_what,_file,_line));
    }
    inline void foutInt(int* _data,int _len,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(IntPtr::make(_data,_len,_what,_file,_line));
    }
    inline void foutInt(int _data,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(IntVar::make(_data,1,_what,_file,_line));
    }
    inline void foutInt(long _data,char* _what,char* _file,int _line){
        if(doDataStream_prior&&doDataStream&&NOT_IN_PARALLEL_REGION) cachedPtr.push_back(IntVar::make(int(_data),1,_what,_file,_line));
    }
    ...
    void flush(){
        // write the file
        if(doDataStream_prior&&doDataStream&&outstream_fn!="NULL")
        {
    ...
    void check(){
        if(doDataStream_prior&&doDataStream&&instream_fn!="NULL")
        {
            if (IN_PARALLEL_REGION && MASTER_THREAD)
            ...
            if (NOT_IN_PARALLEL_REGION)
            {
                ...
                for (auto& ptr : cachedPtr){
                    ...
                    switch (ptr->type()) {
                        case Double:
                            diffcounts = isSame((double*)data,(double*)ptr->data,ptr->len,precision,compare_file);
                            break;
                        case Float:
                            diffcounts = isSame((float*)data,(float*)ptr->data,ptr->len,precision,compare_file);
                            break;
                        case Int:
                            diffcounts = isSame((int*)data,(int*)ptr->data,ptr->len,precision,compare_file);
                            break;
                        default:
                            break;
                    }
                    aFree(data);
                    if (diffcounts!=0) {
                        compare_file<<"!!!!!!diff array..."<<ptr->what<<" in : "<<ptr->file<<" line : "<<ptr->line
                        <<" diff percentage : "<< (double(diffcounts)/double(ptr->len))*100<<"%"<<std::endl<<std::endl;
                    }
                }
            }
        }
